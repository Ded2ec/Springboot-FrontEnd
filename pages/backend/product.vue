<script setup lang="ts">
    ...
    // สร้างตัวแปรไว้สำหรับ Filter ข้อมูล Products
    const filteredProducts: any = ref([])

    // ตัวแปรสำหรับเก็บค่าการค้นหาและหมวดหมู่ที่เลือก
    const search = ref("")
    ...
    // ฟังก์ชันสำหรับจัดรูปแบบวันที่ 2024-08-22T03:57:30.88717 ให้เป็น 22 August 2024
    ...
    // ฟังก์ชันสำหรับจัดรูปแบบตัวเลขยอดเงิน
    const numberWithCommas = (x: number | null | undefined) => {
        if (x === undefined || x === null) {
            return ''; // หรือคุณสามารถเลือกคืนค่าอื่นที่เหมาะสม เช่น '0' หรือ '-'
        }
        return `${x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}฿`;
        
    }

    const numberWith = (x: number | null | undefined) => {
    if (x === undefined || x === null) {
        return ''; // หรือคุณสามารถเลือกคืนค่าอื่นที่เหมาะสม เช่น '0' หรือ '-'
    }
    return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    ...
    const exportCSV = () => {
        ...
        products.value.map((product: any, index: number) => {
            const row = [
                index + 1,
                `"${product.productName.replace(/"/g, '""')}"`, // Handle quotes
                `"${product.categoryName.replace(/"/g, '""')}"`,
                product.unitPrice,
                product.unitInStock,
                (product.createdDate.slice(0, 3).join('-'))
            ]
        ...
        const blob = new Blob([BOM + csvString], { type: 'text/csv;charset=utf-8;' }) // Include BOM for UTF-8
    ...
    const exportPDF = (products: any[]) => {
        ...
        const numberWithCommass = (x: number | null | undefined) => {
    if (x === undefined || x === null) {
        return '0'; // คืนค่า '0' ถ้าค่าเป็น null หรือ undefined
    }
    const numberValue = Number(x); // แปลงเป็น number
    if (isNaN(numberValue)) {
        return '0'; // คืนค่า '0' ถ้าค่าไม่ใช่ตัวเลข
    }
    return `${numberValue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}฿`;
        ...
        const data = products.map((product, index) => [
            ...
            numberWithCommass(product.unitPrice),
            product.unitInStock,
            (product.createdDate.slice(0, 3).join('-'))
        ])
        ...
        doc.autoTable({
            ...
            styles: { font:"Kanit-Regular",fontSize: 10, cellPadding: 5 }, // ขนาดฟอนต์และ padding ของเซลล์
            columnStyles: {
                0: { cellWidth: 30 },
                1: { cellWidth: 'auto' },
                2: { cellWidth: 100 },
    ...
    const fetchProducts = async () => {
        console.log("Fetching products with category:", selectedCategory.value)
        const { data } = await useSpringApi().getAllProducts(
            page.value, 
            rowsPerPage.value, 
            search.value, 
    ...
    const filterProducts = () => {
        console.log("filterProducts called with category:", category.value)
        page.value = 1; // กำหนดหน้าแรกเป็นหน้าที่ 1
        fetchProducts()
    }

    const resetSearch = () => {
        search.value = ''  // รีเซ็ตค่าของ search
        filterProducts()  // เรียกฟังก์ชันเพื่อรีเฟรชข้อมูล
    }
    // ---------------------------------------------------------
                    ...
                    <v-table class="mt-5">
                        ...
                        <tbody>
                            <tr v-for="product in products" :key="product.id">
                                ...
                                <td>{{  formatDate(product.createdDate.slice(0, 3)) }}</td>
                                <td>
                                    <v-icon
                                        small
